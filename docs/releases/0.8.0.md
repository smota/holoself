...
The linked project's `.holoself/link.yaml` remains the authorization boundary and points to the canonical self root even when the request starts in that project folder. The server validates that link on every operation, respects its granted lens, and does not reveal the private data-root path in results or errors.
