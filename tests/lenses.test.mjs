...
test('custom sensitivity is explicit, restricted denied, and registry changes stale the index',async()=>{
  ...
  await run(['index','rebuild','--project',project]);let index=JSON.parse(await readFile(join(project,'.holoself','index','index.json'),'utf8'));assert.equal(index.schema_version,4);assert.equal(index.privacy_policy_version,3);const prior=index.lens_registry_hash
