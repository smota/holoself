...
test('custom sensitivity is explicit, restricted denied, and registry changes stale the index',async()=>{
  ...
  await run(['index','rebuild','--project',project]);let index=JSON.parse(await readFile(join(project,'.holoself','catalog','catalog.json'),'utf8'));assert.equal(index.schema_version,2);const prior=index.lens_registry_hash
  ...
  await capture(()=>run(['search','employermarker','--project',project,'--lens','spiritual']));index=JSON.parse(await readFile(join(project,'.holoself','catalog','catalog.json'),'utf8'));assert.notEqual(index.lens_registry_hash,prior)
})
