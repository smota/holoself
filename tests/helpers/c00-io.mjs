...
export function installIoMeter({ roots, syntheticPaths = [] } = {}) {
  ...
  const install = () => {
    ...
    for (const name of raw) if (typeof fs[name] === 'function') replace(fs, name, original => function (...args) { unsupported(name, 'raw file descriptor operation'); return original.apply(this, args) })
