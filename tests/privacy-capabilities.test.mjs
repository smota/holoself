...
test('incomplete or invalid canonical privacy metadata is excluded from context, index, and search',async()=>{
  ...
  const index=JSON.parse(await readFile(join(self,'.holoself','catalog','catalog.json'),'utf8'))
  assert.equal(index.schema_version,2)
  for(const [name] of cases){
    assert.ok(!index.sources.some(entry=>entry.file===`context/${name}.md`),`${name} index`)
  }
...
test('explicit public_safe false overrides and quarantines conflicting public-safe legacy metadata',async()=>{
  ...
  const index=JSON.parse(await readFile(join(self,'.holoself','catalog','catalog.json'),'utf8'))
  assert.equal(index.schema_version,2)
  assert.ok(!index.sources.some(entry=>entry.file==='context/legacy-conflict.md'))
...
test('index schema tracks freshness and enforces post-build include/exclude assertions',async()=>{
  ...
  await run(['index','rebuild','--project',project]);let index=JSON.parse(await readFile(join(project,'.holoself','catalog','catalog.json'),'utf8'))
  assert.equal(index.schema_version,2);assert.ok(!index.sources.some(entry=>entry.file==='Private/omit.md'))
  const oldHash=index.sources.find(s=>s.file==='Context/keep.md').source_text_hash
  await writeFile(join(project,'Context','keep.md'),'# Keep\n\nfreshnessmarker two.\n')
  ...
  index=JSON.parse(await readFile(join(project,'.holoself','catalog','catalog.json'),'utf8'));assert.notEqual(index.sources.find(s=>s.file==='Context/keep.md').source_text_hash,oldHash)
