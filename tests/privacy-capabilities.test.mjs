...
test('incomplete or invalid canonical privacy metadata is excluded from context, index, and search',async()=>{
  ...
  assert.equal(index.privacy_policy_version,4)
  for(const [name] of cases){
...
test('index schema tracks freshness and enforces post-build include/exclude assertions',async()=>{
  ...
  assert.equal(index.schema_version,5);assert.equal(index.privacy_policy_version,4);assert.equal(index.build_assertions.status,'passed');assert.ok(index.input_state_hash);assert.ok(!index.entries.some(entry=>entry.file==='Private/omit.md'))
  await writeFile(join(project,'Context','keep.md'),'# Keep\n\nfreshnessmarker two.\n')
