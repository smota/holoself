  ...
  try {
    ...
    if (st.isDirectory()) {
      for (const name of readdirSync(path, { recursive: true })) {
        ...
        if (forbidden.test(base) && !base.endsWith('.md') && !base.endsWith('.json') && !base.endsWith('.mjs')) {
          failures.push(`private-looking package path: ${rel}`)
        }
      }
    } else {
      if (forbidden.test(entry) && !entry.endsWith('.md')) {
        failures.push(`private-looking package entry: ${entry}`)
      }
    }
  } catch {
    failures.push(`missing package path: ${entry}`)
  }
}
